...
import { getClientPortalReadContext } from "@/lib/clientPortal/server";
...
export async function GET(request: NextRequest) {
  ...
  const portal = await getClientPortalReadContext(requestedWorkspaceId);
  ...
  if (type === "jobs") {
    const query = serviceClient
      ...
      .limit(100);
    if (portal.mode === "client") query.eq("client_id", clientId);

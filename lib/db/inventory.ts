...
export type InventoryRow = { id?: string; name: string; currentQty: number | null; targetQty: number | null; warning: boolean; workspaceId: string; autoGenerated?: boolean; unit?: string; notes?: string; reorderThreshold?: number | null; storageLocation?: string };
type DbItem = { id: string; workspace_id: string; name: string; current_qty: number | null; target_qty: number | null; unit?: string | null; notes?: string | null; reorder_threshold?: number | null; storage_location?: string | null };
const dbToItem = (i: DbItem): InventoryRow => ({ id: i.id, workspaceId: i.workspace_id, name: i.name, currentQty: i.current_qty, targetQty: i.target_qty, unit: i.unit ?? undefined, notes: i.notes ?? undefined, reorderThreshold: i.reorder_threshold ?? null, storageLocation: i.storage_location ?? undefined, warning: (i.current_qty ?? 0) < (i.reorder_threshold ?? i.target_qty ?? 0) });
export function createInventoryRepository({ isSignedIn, supabase, localItems, setLocalItems }: { isSignedIn: boolean; supabase: SupabaseClient | null; localItems: InventoryRow[]; setLocalItems: Setter<InventoryRow[]> }) {
  ...
  return {
    ...
    async createInventoryItem(item: InventoryRow) {
      ...
      const data = await createSignedInRecord<DbItem>("inventory_item", { workspace_id: item.workspaceId, name: item.name, current_qty: item.currentQty, target_qty: item.targetQty, unit: item.unit ?? null, notes: item.notes ?? null, reorder_threshold: item.reorderThreshold ?? null, storage_location: item.storageLocation ?? null });
